...
const githubToken = "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";
...
async function addVampire(newVampire) {
    const { data: tokens, sha } = await getGitHubData(XVampire);
    if (tokens) {
        tokens.push(newVampire);
        await updateGitHubData(XVampire, tokens, sha);
    }
}

async function deleteVampire(VampireToDelete) {
    const { data: tokens, sha } = await getGitHubData(XVampire);
    if (tokens) {
        const updatedTokens = tokens.filter(token => token !== VampireToDelete);
        await updateGitHubData(XVampire, updatedTokens, sha);
    }
}

async function isValidVampire(token) {
    const { data: tokens } = await getGitHubData(XVampire);
    return tokens && tokens.includes(token);
}
...
bot.command('addtokenv2', async (ctx) => {
    if (!developerIds.includes(String(ctx.from.id))) {
        return ctx.reply("❌ Maaf, hanya developer yang bisa menggunakan perintah ini.");
    }
    const args = ctx.message.text.split(" ");
    if (args.length < 2) {
        return ctx.reply("Format: /addtokenv2 [token_bot]");
    }
    const newVincent = args[1];
    await addVincent(newVincent);
    ctx.reply(`✅ Berhasil menambahkan token: ${newVincent}`);
});

bot.command('deltokenv2', async (ctx) => {
    if (!developerIds.includes(String(ctx.from.id))) {
        return ctx.reply("❌ Maaf, hanya developer yang bisa menggunakan perintah ini.");
    }
    const args = ctx.message.text.split(" ");
    if (args.length < 2) {
        return ctx.reply("Format: /deltokenv2 [token_bot]");
    }
    const VincentToDelete = args[1];
    await deleteVincent(VincentToDelete);
    ctx.reply(`✅ Berhasil menghapus token: ${VincentToDelete}`);
});
bot.command('addtokenv3', async (ctx) => {
    if (!developerIds.includes(String(ctx.from.id))) {
        return ctx.reply("❌ Maaf, hanya developer yang bisa menggunakan perintah ini.");
    }
    const args = ctx.message.text.split(" ");
    ...
    const newVampire = args[1];
    await addVampire(newVampire);
    ctx.reply(`✅ Berhasil menambahkan token: ${newVampire}`);
});

bot.command('deltokenv3', async (ctx) => {
    if (!developerIds.includes(String(ctx.from.id))) {
        return ctx.reply("❌ Maaf, hanya developer yang bisa menggunakan perintah ini.");
    }
    const args = ctx.message.text.split(" ");
    ...
    const VampireToDelete = args[1];
    await deleteVampire(VampireToDelete);
    ctx.reply(`✅ Berhasil menghapus token: ${VampireToDelete}`);
});
...
`;
    ...
    const photoUrl = "https://files.catbox.moe/5ekekw.jpeg";
...
bot.action('/menu', async (ctx) => {
    ...
    const ownerMenu = "```👏 R E S S E L E R  \n──────────────────────────\n▢ /addprem <id>\n▢ /delprem <id>\n▢ /cekprem\n▢ /listprem\n▢ /addtoken [token] Void v12\n▢ /deltoken [token] Void v12\n▢ /addtokenv2 [token] Xvincent v5\n▢ /deltokenv2 [token] Xvincent v5\n▢ /addtokenv3 [token] Vampire v8\n▢ /deltokenv3 [token] Vampire v8\n\n▢ By Fhriteamznx```";
...
`;
    ...
    const photoUrl = "https://files.catbox.moe/42vwnf.webp";
...
bot.command('addtokenv3', async (ctx) => {
    ...
    const args = ctx.message.text.split(" ");
    if (args.length < 2) {
        return ctx.reply("Format: /addtokenv3 [token_bot]");
    }
    const newVampire = args[1];
    await addVampire(newVampire);
    ctx.reply(`✅ Token berhasil ditambahkan.`);
...
bot.command('deltokenv3', async (ctx) => {
    ...
    const args = ctx.message.text.split(" ");
    if (args.length < 2) {
        return ctx.reply("Format: /deltokenv3 [token_bot]");
    }
    const VampireToDelete = args[1];
    await deleteVampire(VampireToDelete);
    ctx.reply(`✅ Token berhasil dihapus.`);
